...
# mcdaniel: handles setup.py install where a required import in __init__.py
# fails to import this module because most of the imports are not included in requirements
if import_success:
    ...
    class SWPWRXBlock(StudioEditableXBlockMixin, ScorableXBlockMixin, XBlock):
        ...
        q_type = String(
            help="SWPWR Type", default="gradeBasicAlgebra", scope=Scope.content
        )
        ...
        grade = Float(
            help="SWPWR The student's grade", default=-1, scope=Scope.user_state
        )
        ...
        def student_view(self, context=None):
            ...
            try:
                temp_course_stepwise_grade_hints_count = (
                    course.stepwise_grade_hints_count
                )
            except (NameError, AttributeError) as e:
                if DEBUG:
                    logger.info(
            ...
            try:
                temp_course_stepwise_grade_errors_count = (
                    course.stepwise_grade_errors_count
                )
            except (NameError, AttributeError) as e:
                if DEBUG:
                    logger.info(
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock student_view() self.my_weight={m}".format(
                        m=self.my_weight
                    )
                )
            ...
            if temp_grade_min_steps_count != -1:
                ...
                self.my_grade_min_steps_count = (
                    temp_course_stepwise_grade_min_steps_count
                )
            else:
                self.my_grade_min_steps_count = (
                    def_course_stepwise_grade_min_steps_count
                )
            if DEBUG:
                logger.info(
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock student_view() KENT my_test_url={r}".format(
                        r=my_test_url
                    )
                )
            ...
            frag.add_resource(
                "<title>Querium StepWise Power</title>", "text/html", "head"
            )
            ...
            if TEST_MODE:
                ...
                if DEBUG:
                    logger.info(
                        "SWPWRXBlock student_view() swpwr_string={e}".format(
                            e=swpwr_string
                        )
                    )
        ...
        def save_grade(self, data, suffix=""):
            ...
            if q_grade_errors_count == -1:
                if DEBUG:
                    logger.info(
                        "SWPWRXBlock save_grade() errors_count default set to 3"
                    )
                q_grade_errors_count = 3
            if q_grade_errors_ded == -1:
                if DEBUG:
                    logger.info(
                        "SWPWRXBlock save_grade() errors_ded default set to 1.0"
                    )
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock save_grade() final self.weight={a}".format(
                        a=self.weight
                    )
                )
            if DEBUG:
        ...
        def start_attempt(self, data, suffix=""):
            ...
            if self.bit_is_set(self.variants_attempted, variant):
                if DEBUG:
                    logger.info(
                        "variant {v} has already been attempted!".format(v=variant)
                    )
            else:
                if DEBUG:
                    logger.info(
                ...
                self.variants_attempted = self.bit_set_one(
                    self.variants_attempted, variant
                )
                if DEBUG:
                    logger.info(
        ...
        def author_view(self, context=None):
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock SWPWRXAuthor author_view v={a}".format(
                        a=self.q_definition
                    )
                )
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock SWPWRXAuthor author_view variants={a}".format(
                        a=variants
                    )
                )
        ...
        def save_swpwr_final_results(self, data, suffix=""):
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock save_swpwr_final_results() back from save_grade"
                )
            return {"result": "success"}
        ...
        def save_swpwr_partial_results(self, data, suffix=""):
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock save_swpwr_partial_results() back from save_grade"
                )
            return {"result": "success"}
        ...
        def get_score(self):
            ...
            if DEBUG:
                ...
                logger.info(
                    "SWPWRXBlock get_score() earned {e}".format(e=self.raw_earned)
                )
            if DEBUG:
                logger.info(
                    "SWPWRXBlock get_score() max {m}".format(m=self.max_score())
                )
        ...
        def set_score(self, score):
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock set_score() earned {e}".format(e=score.raw_earned)
                )
        ...
        def calculate_score(self):
            ...
            if DEBUG:
                logger.info(
                    "SWPWRXBlock calculate_score() grade {g}".format(g=self.grade)
                )
            if DEBUG:
                logger.info(
